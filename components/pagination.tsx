...
export function Pagination({ currentPage, totalPages, basePath }: PaginationProps) {
  ...
  const pageLinkClassName =
    "inline-flex h-9 min-w-9 items-center justify-center border px-2.5 font-mono text-xs uppercase tracking-[0.12em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background";
  const edgeLinkClassName =
    "inline-flex h-9 items-center gap-1 border border-border px-3 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background";
  const disabledClassName =
    "inline-flex h-9 items-center gap-1 border border-border/50 px-3 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground/40";
  ...
  return (
    ...
    >
      ...
      <div className="hidden items-center gap-1 sm:flex">
        {items.map((item, index) =>
          item === "ellipsis" ? (
            <span
              key={`ellipsis-${index}`}
              className="inline-flex h-9 min-w-9 items-center justify-center px-2 font-mono text-xs text-muted-foreground/60"
              aria-hidden="true"
            >
              ...
            </span>
      ...
      <span className="inline-flex h-9 min-w-20 items-center justify-center border border-border px-3 font-mono text-xs text-muted-foreground sm:hidden">
